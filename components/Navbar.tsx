  ...
  return (
    <>
      <div className="w-full flex justify-center sticky top-4 md:top-8 z-50 mb-4 md:mb-8 px-4 sm:px-6 md:px-16">
        <header className="w-full max-w-7xl bg-white/95 backdrop-blur-md border border-[#e4e2e2] rounded-full px-4 sm:px-6 py-2.5 md:py-3 shadow-md flex justify-between items-center transition-all duration-300">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            {/* Badge */}
            <div className="hidden md:flex items-center gap-2 bg-surface-container border border-outline-variant px-3 py-1.5 rounded-full hover:border-primary/50 transition-colors">
              <div className="w-2 h-2 rounded-full bg-primary animate-pulse"></div>
              <span className="text-[11px] font-bold text-on-surface tracking-widest uppercase">JEPARA {mounted ? timeString : "..."}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {activeView !== "admin" ? (
              <>
                {/* Cart Icon Button */}
                <button onClick={() => setIsCartOpen(true)} aria-label="shopping_bag" className="flex items-center justify-center w-10 h-10 rounded-xl bg-surface-container-low border border-outline-variant text-secondary hover:text-primary hover:bg-surface-container-high hover:scale-105 active:scale-95 transition-all relative">
                  <span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: "'FILL' 0" }}>shopping_bag</span>
                  {totalItemCount > 0 && (
                    <span className="absolute -top-1 -right-1 bg-primary text-on-primary text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center animate-bounce">{totalItemCount}</span>
                  )}
                </button>
                ...
                <div className="hidden md:block relative group">
                  {mounted ? (
                    user ? (
                      <button type="button" onClick={() => setIsLogoutModalOpen(true)} className="inline-flex items-center gap-1.5 px-4 py-2 bg-surface-container-high border border-outline-variant text-primary font-bold rounded-full text-xs hover:bg-red-600 hover:text-white transition-all shadow-sm active:scale-95">
                        <span className="material-symbols-outlined text-[17px]">account_circle</span>
                        <span className="capitalize max-w-[120px] truncate">{user.displayName || user.email?.split("@")[0]}</span>
                        <span className="material-symbols-outlined text-[14px]">logout</span>
                      </button>
                    ) : (
                      <Link href="/login" className="inline-flex items-center gap-1.5 px-4 py-2 bg-black text-white font-bold rounded-full text-xs hover:bg-neutral-800 transition-all shadow-sm active:scale-95">
                        <span className="material-symbols-outlined text-[17px]">account_circle</span>
                        <span>Masuk</span>
                      </Link>
                    )
                  ) : (
                    <div className="inline-flex items-center gap-1.5 px-4 py-2 bg-surface-container-high border border-outline-variant text-on-surface-variant font-bold rounded-full text-xs">
                      <span className="animate-spin inline-block w-4 h-4 border-2 border-current border-t-transparent text-primary rounded-full" />
                    </div>
                  )}
                </div>
              </>
